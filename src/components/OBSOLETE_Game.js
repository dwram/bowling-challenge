...
class Game extends React.Component{
    ...
    render() {
        return (
            <h1>
                // eslint-disable-next-line
                {Object.entries(this.frames).map(([frame, score]) => {
                //console.log("Frame: " + frame + " | Score: " + score);
                if (frame !== '0') {
                    if (frame !== '11') {
                        return < div className="box-field">
                        Frame: {frame} | Pins: {score.toString()}
                        </div>
                    } else if (frame === '11' && score > 0 ) {
                        return < div className="box-field-bonus">
                        Bonus frame: {frame} | Pins: {score.toString()}
                        </div>
                    }
                }
            })};
